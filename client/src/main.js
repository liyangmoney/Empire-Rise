...
function connect() {
  ...
  socket.on('map:castlePlaced', (data) => {
    showSuccess('城堡已放置！位置: (' + data.position.x + ', ' + data.position.y + ')');
  });
  ...
  socket.on('map:tradeOptions', (data) => {
    console.log('Trade options:', data);
    showTradeOptions(data);
  });

  socket.on('map:battleStarted', (data) => {
    console.log('Map battle started:', data);
    showInfo('战斗开始！对阵 ' + data.npc.name + ' (战力:' + data.npc.power + ')');
  });
...
function _old_renderMiniMap(fullMap) {
  ...
  fullMap.npcs.forEach(npc => { ctx.fillRect(npc.x * scale, npc.y * scale, scale * 2, scale * 2); });
  fullMap.castles.forEach(castle => { ctx.fillStyle = castle.isOwn ? '#8B4513' : '#4a4a4a'; ctx.fillRect(castle.x * scale - 1, castle.y * scale - 1, scale * 3, scale * 3); });
  const myCastle = fullMap.castles.find(c => c.isOwn);
  if (myCastle) { ctx.strokeStyle = '#FF0000'; ctx.lineWidth = 1; const viewRadius = 10; ctx.strokeRect((myCastle.x - viewRadius) * scale, (myCastle.y - viewRadius) * scale, viewRadius * 2 * scale, viewRadius * 2 * scale); }
}
...
function showTileInfo(tile) {
  ...
  if (tile.npcs && tile.npcs.length > 0) {
    ...
    tile.npcs.forEach((npc, index) => {
      npcHtml += '<li style="margin: 5px 0; padding: 5px; background: rgba(0,0,0,0.3); border-radius: 4px;">' + (npc.isNeutral ? '🏪' : '⚔️') + ' ' + npc.name + ' ' + (npc.power ? '(战力: ' + npc.power + ')' : '') + (!npc.isNeutral ? '<button onclick="attackMapNpc(' + tile.x + ', ' + tile.y + ', ' + index + ')" style="margin-left: 10px; padding: 2px 8px; font-size: 12px;">攻击</button>' : '') + (npc.isNeutral ? '<button onclick="tradeWithNpc(' + tile.x + ', ' + tile.y + ', ' + index + ')" style="margin-left: 10px; padding: 2px 8px; font-size: 12px;">交易</button>' : '') + '</li>';
    });
    npcHtml += '</ul>';
  }
  info.innerHTML = '<p><strong>位置:</strong> (' + tile.x + ', ' + tile.y + ')</p><p><strong>地形:</strong> ' + (tile.terrain.name || tile.terrain.id) + '</p>' + (tile.terrain.defenseBonus !== 0 ? '<p><strong>防御加成:</strong> ' + (tile.terrain.defenseBonus > 0 ? '+' : '') + tile.terrain.defenseBonus + '%</p>' : '') + (tile.hasCastle ? '<p style="color: #ffd700;">🏰 这里有一座城堡</p>' : '') + npcHtml + (!tile.hasCastle && !tile.npcs?.length ? '<button onclick="migrateCastleTo(' + tile.x + ', ' + tile.y + ')" style="margin-top: 10px;">迁移城堡至此</button>' : '');
}

function attackMapNpc(x, y, npcIndex) {
  if (!socket || !playerId) { showError('请先连接服务器'); return; }
...
function showTradeOptions(data) {
  ...
  data.offers.forEach((offer, index) => {
    ...
    offersHtml += '<li style="margin: 10px 0; padding: 10px; background: rgba(0,0,0,0.3); border-radius: 4px; display: flex; justify-content: space-between; align-items: center;"><span>给出: ' + giveText + ' → 获得: ' + getText + '</span><button onclick="executeTrade(' + index + ')" style="padding: 2px 8px; font-size: 12px;">交易</button></li>';
  });
  offersHtml += '</ul>';
  info.innerHTML = '<p><strong>🏪 ' + data.merchant + '</strong></p>' + offersHtml;
}

function getResourceName(resId) {
  const names = { wood: '木材', stone: '石材', food: '粮食', iron: '铁矿', crystal: '水晶', gold: '金币', fish_product: '鱼产品', fruit: '水果', premium_food: '精品食材' };
  return names[resId] || resId;
}

function executeTrade(offerIndex) { showInfo('交易功能开发中...'); }
